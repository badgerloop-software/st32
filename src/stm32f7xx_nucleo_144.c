#include "../include/stm32f7xx_nucleo_144.h"
...
void LCD_IO_WriteMultipleData(uint8_t *pData, uint32_t Size) {
  ...
  LCD_CS_LOW();		/* Reset LCD control line CS */
  LCD_DC_HIGH();	/* Set LCD data/command line DC to High */

  if (Size == 1) 
    /* Only 1 byte to be sent to LCD - general interface can be used */
    SPIx_Write(*pData);	/* Send Data */
  else {
...
JOYState_TypeDef BSP_JOY_GetState(void) {
  ...
  HAL_ADC_Start(&hnucleo_Adc);	/* Start the conversion process */
  
  HAL_ADC_PollForConversion(&hnucleo_Adc, 10);	/* Wait for the end of conversion */
